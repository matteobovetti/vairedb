...
| **Push-Down** | Moving computation closer to the data. DataFusion moves it down the plan; VaireDB takes the last step, carrying projections and the filters and row limits DuckDB reads identically into each shard's own `SELECT`. A bandwidth optimization — the coordinator re-applies both, so it never changes an answer. |
